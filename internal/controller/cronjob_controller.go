...
func (r *CronJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// delete the old jobs on best-effort basis
	...
	if cronJob.Spec.SuccesfulJobHistoryLimit != nil {
		...
		for i, job := range successfulJobs {
			if int32(i) >= int32(len(successfulJobs))-*cronJob.Spec.SuccesfulJobHistoryLimit {
				break
			}
			if err := r.Delete(ctx, job, client.PropagationPolicy(metav1.DeletePropagationBackground)); err != nil {
	...
	getNextSchedule := func(cronJob *batchv1.CronJob, now time.Time) (last, next time.Time, err error) {
		...
		if earliestTime.After(now) {
			return time.Time{}, sched.Next(earliestTime), nil
		}
	...
	if err != nil {
		log.Error(err, "unable to figure CronJob schedule")
		return ctrl.Result{}, err
	}
